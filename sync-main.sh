#!/usr/bin/env bash
# Sync development branch to main with squashed commit
...
Syncs source to main with squashed commit, then merges back for traceability.
...
Workflow:
  1. Checkout main
  2. Copy source → main (excluding patterns)
  3. Commit: "import from <source>"
  4. Merge main → source (traceability)
EOF
}
...
if [ "$AUTO_YES" = false ] && [ "$DRY_RUN" = false ]; then
    ...
    if [ "$IS_BRANCH" = true ]; then
        echo "  3. Merge '$MAIN_BRANCH' back into '$SOURCE_REF' (for traceability)"
    else
...
if [ "$DRY_RUN" = false ]; then
    ...
    echo -e "${BLUE}[DRY-RUN]${NC} git add -A"
fi
...
if [ "$CURRENT_BRANCH" != "detached" ] && [ "$CURRENT_BRANCH" != "$SOURCE_REF" ]; then
    echo -e "${GREEN}Step 7: Return to original branch: $CURRENT_BRANCH${NC}"
